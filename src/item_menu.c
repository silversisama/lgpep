...
void ResetBagScrollPositions(void)
{
    ...
    gBagPosition.pocket = POCKET_HEALING_ITEMS;
...
{
    GoToBagMenu(ITEMMENULOCATION_BERRY_TREE_MULCH, POCKET_MISC_ITEMS, CB2_ReturnToFieldContinueScript);
}
...
static void OpenContextMenu(u8 taskId)
{
    ...
    if (GetItemBattleUsage(gSpecialVar_ItemId) && FlagGet(FLAG_NUZLOCKEHC) 
        && gBagPosition.pocket != POCKET_POKE_BALLS && gBagPosition.location == ITEMMENULOCATION_BATTLE)
            ...
            StringExpandPlaceholders(gStringVar4, gText_Var1IsSelected);        FillWindowPixelBuffer(WIN_DESCRIPTION, PIXEL_FILL(0));
        BagMenu_Print(WIN_DESCRIPTION, FONT_NORMAL, gStringVar4, 3, 1, 0, 0, 0, COLORID_NORMAL);
    }
...
static void PrepareBagForWallyTutorial(void)
{
    ...
    memcpy(sTempWallyBag->bagPocket_HealingItems, gSaveBlock1Ptr->bag.HealingItems, sizeof(gSaveBlock1Ptr->bag.HealingItems));
    memcpy(sTempWallyBag->bagPocket_HeldItems, gSaveBlock1Ptr->bag.heldItems, sizeof(gSaveBlock1Ptr->bag.heldItems));
    memcpy(sTempWallyBag->bagPocket_MiscItems, gSaveBlock1Ptr->bag.miscItems, sizeof(gSaveBlock1Ptr->bag.miscItems));
...
static void RestoreBagAfterWallyTutorial(void)
{
    ...
    memcpy(gSaveBlock1Ptr->bag.HealingItems, sTempWallyBag->bagPocket_HealingItems, sizeof(sTempWallyBag->bagPocket_HealingItems));
    memcpy(gSaveBlock1Ptr->bag.heldItems, sTempWallyBag->bagPocket_HeldItems, sizeof(sTempWallyBag->bagPocket_HeldItems));
    memcpy(gSaveBlock1Ptr->bag.miscItems, sTempWallyBag->bagPocket_MiscItems, sizeof(sTempWallyBag->bagPocket_MiscItems));
...
void DoWallyTutorialBagMenu(void)
{
    ...
    GoToBagMenu(ITEMMENULOCATION_WALLY, POCKET_HEALING_ITEMS, CB2_SetUpReshowBattleScreenAfterMenu2);
}
